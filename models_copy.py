...
class Plan_GRU(torch.nn.Module):
    ...
    def forward(self, input_hidden, tgt, max_length=30, teacher_forcing_ratio=0.5, pos_label=None, generate=False):
        '''
        ...
        predictions = []
        infer = []
        flag = torch.zeros(input_hidden.shape[0], input_hidden.shape[1], device=input_hidden.device) #batch, len
...
class BartModel(PretrainedBartModel):
    ...
    def forward(
        ...
        if self.args.planning and input_hidden_states is None:
            ...
            out = self.planning_net(input_hidden=nodes_emb, tgt=target_emb, teacher_forcing_ratio=self.args.teacher_force, pos_label=pos_label)
